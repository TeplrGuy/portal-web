...
permissions:
  ...
  issues: read
  pull-requests: read
tracker-id: portal_web-nightly-quality
safe-outputs:
  create-issue:
...
You are the ambient quality agent for `portal-web`. Your job is to surface regressions, friction, and drift before they compound. Create one quality issue per day summarizing the repository health — be specific, link to evidence, and flag anything that needs human action today.
